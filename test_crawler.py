...
def list_jobs(url):
    ...
    for i in range(len(vks)):
        ...
        try:
            p = multiprocessing.Process(target=download, args=(url, title, filetype))
    return jobs
...
def detail_page(url):
    ...
    config = json.loads(tem)

    for dic in config['mediaDefinitions']:
...
def download(url, title, filetype):
    ...
    urllib.request.urlretrive(url, r'{}/{}.{}'.format(filetype, title, filetype))
    end_time = time.time()
